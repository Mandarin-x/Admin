...
public class AdminDaoImpl implements AdminDao{
	...
	public boolean login(String id, String pwd) {
		...
		try {
			DBconn.init();
			ResultSet rs = DBconn.selectSql("select * from admin_ where ad_id='"+ id +"' and ad_password='"+pwd+"'");
			while(rs.next()) {
				...
				if (0 == rs.getInt("ad_state")) {
					int i = DBconn.addUpdDel("update admin_ set ad_state=1 where ad_id='"+id+"'");
					if (i>0) {
						admin.setState(rs.getInt("ad_state"));
						flag = true;
					}
	...
	public boolean logout(String id) {
		...
		try {
			DBconn.init();
			ResultSet rs = DBconn.selectSql("select * from admin_ where ad_id='"+ id +"'");
			
			while(rs.next()) {
				if (1 == rs.getInt("ad_state")) {
					
					int i = DBconn.addUpdDel("update admin_ set ad_state=0 where ad_id='"+id+"'");
					if(i>0) {
						flag = true;
					}
				}else {
	...
	public List<Admin> getAllAdmin() {
		...
		try {
			DBconn.init();
			ResultSet rs = DBconn.selectSql("select * from admin_");
			while(rs.next()) {
				Admin ad = new Admin();
